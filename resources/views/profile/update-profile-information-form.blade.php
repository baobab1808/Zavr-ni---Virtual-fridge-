<x-jet-form-section submit="updateProfileInformation">
    <x-slot name="title" >
       <h1 style="font-weight:bold; font-size:30px; font-family:'Open Sans'"> {{ __('Podaci o profilu') }}</h1>
    </x-slot>

    <x-slot name="description">
       <h3> {{ __('Ažurirajte podatke o profilu i email svog računa') }} </h3>
    </x-slot>
        ...
        @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
            <div class="form-group" x-data="{photoName: null, photoPreview: null}">
                ...
                <x-jet-secondary-button class="mt-2 mr-2" type="button" x-on:click.prevent="$refs.photo.click()">
                    {{ __('Odaberi novu fotografiju') }}
				</x-jet-secondary-button>
				
				@if ($this->user->profile_photo_path)
                    <x-jet-secondary-button type="button" class="mt-2" wire:click="deleteProfilePhoto">
                        {{ __('Ukloni fotografiju') }}
                    </x-jet-secondary-button>
                @endif
